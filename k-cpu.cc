...
void cpustate::init() {
    // Note that the `cpu::cpu` constructor has already been called.
    {
        ...
        if (!vm_) {
            ...
            assert(this >= cpus_vm && this < cpus_vm + MAXVMCPU);            
        }
...
void cpustate::schedule(proc* yielding_from) {
    ...
    if (current_ && current_->exiting_) {
        current_->pstate_ = proc::ps_exited;
        current_ = nullptr;
    }
    ...
    while (!current_
        ...
        if (prev && prev->pstate_ == proc::ps_runnable && !prev->vm_booter_) {
