...
void memusage::refresh() {
    ...
    for (auto i = 0; i < ncpu; ++i) {
        if (cpus[i].idle_task_ != nullptr) {
            mark(kptr2pa(cpus[i].idle_task_), f_kernel);
        }
    }

    if (sata_disk != nullptr) {
        for (auto i = 0; i < 4; ++i) {
            mark(ka2pa(sata_disk) + PAGESIZE * i, f_kernel);
        }
    }
    ...
    for (int pid = 1; pid < NPROC; ++pid) {
        ...
        if (p) {
            ...
            irqs = fdtable_lock.lock();
            if (p->fdtable_ != nullptr) {
                mark(ka2pa(p->fdtable_), f_kernel | f_process(pid)); 
                for (fd_t i = 0; i < NFD; ++i) {
                    if (p->fdtable_->fds_[i] != nullptr) {
                        auto v = p->fdtable_->fds_[i]->v_;
                        mark(ka2pa(v), f_kernel | f_process(pid));
                        if (v->get_bb() != nullptr) {
                            mark(ka2pa(v->get_bb()), f_kernel | f_process(pid));
                        }
                    }
                }
            }
...
void console_memviewer(proc* vmp) {
    ...
    auto const show_vm = vmp && vmp->vm_btime_;
    auto const now = ticks.load();
    if (!show_vm) {
