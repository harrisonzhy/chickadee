...
#define VPTE_W          0b10
#define VPTE_X          0b100
#define VPTE_PW         (VPTE_W ^ VPTE_P)
#define VPTE_PWX        (VPTE_W ^ VPTE_P ^ VPTE_X)
#define VDIRTYACCESS    (1 << 6)
#define VPTE_PS         0x10000000
#define VPTE_AD         ((1 << 8) ^ (1 << 9))

// intel figure 28.1
x86_64_pagetable  __section(".lowdata") vm_pagetable[4] __attribute__((aligned(PAGESIZE)));
uintptr_t get_ept_ptr() {
    uintptr_t p = 0;
    p ^= 6;             // write back (cacheable)
    p ^= (0b11 << 3);   // page walk length minus 1
    p ^= (1 << 6);      // access and dirty flags
    p ^= reinterpret_cast<uint64_t>(vm_pagetable) / PAGESIZE;
    return p;
}

void init_vm_memory() {
    memset(vm_pagetable, 0, sizeof(vm_pagetable));
    vm_pagetable[0].entry[0] = (ktext2pa(&vm_pagetable[1]) / PAGESIZE) | VPTE_PWX;
    // map the two 2 MiB pages (PML2)
    //  we only have 4 MiB pages of memory
    vm_pagetable[1].entry[0] = (ktext2pa(&vm_pagetable[2]) / PAGESIZE) | VPTE_PWX | VPTE_PS;
    vm_pagetable[1].entry[1] = (ktext2pa(&vm_pagetable[3]) / PAGESIZE) | VPTE_PWX | VPTE_PS;

    for (auto i = 0; i < 16; ++i) {
        auto pg = reinterpret_cast<uint64_t>(kalloc(PAGESIZE));
        memset(reinterpret_cast<void*>(pg), 0xF4, PAGESIZE);
        assert(pg != 0);
        vm_pagetable[2].entry[i] = (ka2pa(pg) / PAGESIZE) | VPTE_PWX;
    }
}
...
void cpustate::init_cpu_hardware() {
    ...
    // schedule cpu
    if (vm_) {
        // %rsp must be within this `cpustate`
        console_printf("%%rsp=%p\n", rdrsp());
        uint64_t new_rsp = reinterpret_cast<uint64_t>(this) + CPUSTACK_SIZE - 8;
        asm volatile(
            "movq %0, %%rsp;"
            :: "r" (new_rsp));
        console_printf("%%rsp=%p\n", rdrsp());

        uintptr_t delta = rdrsp() - reinterpret_cast<uintptr_t>(this);
        console_printf("(VMINFO): delta=%p, CPUSTACK_SIZE=%p\n", delta, CPUSTACK_SIZE);
        assert(delta <= CPUSTACK_SIZE); 
