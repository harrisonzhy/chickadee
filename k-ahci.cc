...
int ahcistate::read_or_write(idecommand command, void* buf, size_t sz,
                             size_t off, std::atomic<int>& fetch_status) {
    ...
    for (unsigned i = 0; i < nslots_; ++i) {
        if (!(slots_outstanding_mask_ & (1UL << i))) {
            slot = i;
            break;
        }
    }
    ...
    waiter().block_until(wq_, [&] () {
            return fetch_status != E_AGAIN;
        }, guard);
    return fetch_status;
}


int ahcistate::read_or_write_nonblocking(idecommand command, void* buf, size_t sz,
                             size_t off, std::atomic<int>& fetch_status) {
    // `sz` and `off` must be sector-aligned
    assert(sz % sectorsize == 0 && off % sectorsize == 0);
    ...
    // return error if command cannot be queued
    if (slot == nslots_) {
        lock_.unlock(irqs);
        return -1;
    }
