...
struct __attribute__((aligned(4096))) proc {
    enum pstate_t {
        ps_blank = 0, ps_runnable = PROC_RUNNABLE, ps_faulted, ps_blocked, ps_exited
    };
    ...
    bool vm_booter_ = false;
    proc* vm_proc_ = nullptr;
    std::atomic<unsigned long> vm_btime_ = 0;

    list_links child_link_;
    list<proc, &proc::child_link_> children_;
    ...
    uintptr_t syscall_read(regstate* regs);
    uintptr_t syscall_write(regstate* regs);
    uintptr_t syscall_readdiskfile(regstate* regs);

    uintptr_t syscall_waitpid(pid_t pid, irqstate& irqs, int* stat = nullptr, int options = 0);
    int syscall_msleep(regstate* regs);
    int syscall_map_console(regstate* regs);
    void syscall_shityourself(regstate* regs);
    int syscall_npagealloc(regstate* regs);
    void syscall_free(regstate* regs);
    ...
    int syscall_gettid(regstate* regs);
    [[noreturn]] void syscall_texit(regstate* regs);

    void syscall_vmx(regstate* regs);
    int syscall_testsmalloc(regstate* regs);
...
struct __attribute__((aligned(4096))) cpustate {
    ...
    inline cpustate()
        : self_(this), vm_(false), vm_cpu_(nullptr), real_cpu_(nullptr), canary_(CANARY) {
    }
...
extern cpustate cpus_vm[MAXVMCPU];
extern int ncpu;
...
public:
    ...
    ~stack_canary() {
        assert(proc_->canary1_ == CANARY);
        assert(proc_->canary2_ == CANARY);
        check_canary();
    }
    ...
    void check_canary() {
        ...
        for (auto i = 0; i < MAXVMCPU; ++i) {
            assert(cpus_vm[i].canary_ == CANARY);
        }
    }
private:
    proc* proc_;
...
// Virtual memory size
#define MEMSIZE_VIRTUAL         0x600000
...
template <typename T>
inline bool is_pa(T* ptr) {
    uintptr_t pa = reinterpret_cast<uint64_t>(ptr);
    return 0 < pa && pa < MEMSIZE_PHYSICAL;
}
...
void* _kalloc(size_t sz) __attribute__((malloc));
...
// Start vm kernel (can fail and return)
