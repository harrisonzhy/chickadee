...
int generic_file::validate_name(const char* name) {
    if (name != nullptr) {
        for (unsigned i = 0; i < generic_file::namesize; ++i) {
            unsigned long res;
            auto addr = reinterpret_cast<uintptr_t>(&name[i]);
            if (__builtin_uaddl_overflow(addr, 1, &res)
             || !vmiter(current(), addr).range_perm(1, PTE_P | PTE_U)) {
                return E_FAULT;
            }
            if (name[i] == '\0') {
                return i;
...
int generic_file::validate_argv(char** argv, int argc) {
    if (!argv || argc < 1) {
        return E_FAULT;
    }
    int len = 0;
    for (auto i = 0; i < argc; ++i) {
        auto r = validate_name(argv[i]);
        if (r < 0) {
            return E_FAULT;
        } else {
            len += (r + 1);
    ...
    while (nread < PAGESIZE) {
        ...
        if (auto e = it.find(off).load()) {
            ...
            size_t ncopy = min(
                ...
                PAGESIZE - nread                    // bytes left in request
            );
