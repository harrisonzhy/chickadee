...
//    Map console memory
inline void sys_map_console(volatile void* addr) {
   make_syscall(SYSCALL_MAP_CONSOLE, reinterpret_cast<uintptr_t>(addr)); 
}

[[noreturn]] inline void sys_shit_yourself() {
    make_syscall(SYSCALL_SHITYOURSELF);
...
inline int sys_npage_alloc(void* addr, size_t size) {
    return make_syscall(SYSCALL_NPAGE_ALLOC, reinterpret_cast<uintptr_t>(addr), size); 
}

inline void sys_free(void* addr) {
    make_syscall(SYSCALL_FREE, reinterpret_cast<uint64_t>(addr));
}

inline int sys_test_smalloc() {
    return make_syscall(SYSCALL_TESTSMALLOC);
}
...
inline int sys_waitpid(pid_t pid, int* status = nullptr, int options = 0) {
    clobber_memory(status);
    uint64_t r = make_syscall(SYSCALL_WAITPID, pid,
                        reinterpret_cast<uintptr_t>(status), options);
    if (status != nullptr) {
        *status = (r >> 32) & 0xFFFF'FFFF; 
    }
    int const r_ = (r & 0xFFFF'FFFF) + E_MINERROR;
    return r_;
}
