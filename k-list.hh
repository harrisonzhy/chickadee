...
struct list {
    ...
    // Returns `true` iff this list contains `x` and false otherwise.
...
template <typename T, list_links (T::* member)>
inline bool list<T, member>::contains(T* x) {
    for (auto x_ = front(); x_ != nullptr; x_ = next(x_)) {
        if (x_ == x) {
            return true;
        }
    }
    return false; // false for nullptr
}
