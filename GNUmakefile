...
ifeq ($(VMX),1)
QEMUOPT += -enable-kvm -cpu host
endif

ifeq ($(D),3)
QEMUOPT += -d cpu_reset -D qemu.log
endif
...
KERNEL_OBJS = $(OBJDIR)/k-exception.ko \
	...
	$(OBJDIR)/k-alloc.ko $(OBJDIR)/k-alloc-def.ko $(OBJDIR)/k-vmiter.ko $(OBJDIR)/k-devices.ko \
...
ifeq ($(MININITFS),1)
INITFS_CONTENTS += obj/p-$(CHICKADEE_FIRST_PROCESS)
else
INITFS_CONTENTS += $(patsubst %,obj/%,$(PROCESSES))
endif
