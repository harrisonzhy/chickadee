...
// read {c,d,e,f,g,s}s register selectors
__always_inline uint16_t rdcs() {
    uint16_t x;
    asm volatile("mov %%cs, %0" : "=r" (x));
    return x;
}
__always_inline uint16_t rdss() {
    uint16_t x;
    asm volatile("mov %%ss, %0" : "=r" (x));
    return x;
}
__always_inline uint16_t rdds() {
    uint16_t x;
    asm volatile("mov %%ds, %0" : "=r" (x));
    return x;
}
__always_inline uint16_t rdes() {
    uint16_t x;
    asm volatile("mov %%es, %0" : "=r" (x));
    return x;
}
__always_inline uint16_t rdfs() {
    uint16_t x;
    asm volatile("mov %%fs, %0" : "=r" (x));
    return x;
}
__always_inline uint16_t rdgs() {
    uint16_t x;
    asm volatile("mov %%gs, %0" : "=r" (x));
    return x;
}

// https://forum.osdev.org/viewtopic.php?f=1&t=33739
__always_inline uint16_t rdtr() {
    uint16_t x;
    asm volatile("str %[x]" : [x]"=rm" (x));
    return x;
}
...
__always_inline int vmwrite(uint64_t encoding, uint64_t value) {
    uint8_t flag;
	asm volatile(
        "vmwrite %[value], %[encoding]; setna %[flag]"
		: [flag] "=rm" (flag)
...
// `setna` checks if rflags.cf == 0 (success)
__always_inline int vmxon(uint64_t region) {
    uint8_t flag;
    asm volatile(
        "vmxon %[pa]; setna %[flag]"
        : [flag] "=rm" (flag)
...
__always_inline int vmptrld(uint64_t region){
    uint8_t flag;
	asm volatile(
        "vmptrld %[pa]; setna %[flag]"
		: [flag] "=rm" (flag)
		: [pa] "m" (region)
		: "cc", "memory");
	return flag;
...
__always_inline int vmread_(uint64_t encoding, uint64_t& value) {
	uint8_t flag;
	uint64_t tmp;
	asm volatile(
		"vmread %[encoding], %[value]; setna %[flag]"
		: [value]"=rm"(tmp), [flag]"=rm"(flag)
		: [encoding]"r"(encoding)
		: "cc", "memory"
	);
    value = tmp;
...
__always_inline uint64_t vmread(uint64_t encoding) {
    uint64_t val = 0;
    (void) vmread_(encoding, val);
    return val;
}

__always_inline uint64_t segmentlimit(const uint64_t value) {
  uint64_t x;
  asm volatile("lsl %[value], %[x]"
                : [x] "=r" (x)
                : [value] "rm" (value));
  return x;
}

__always_inline uint16_t rdldtr() {
    uint16_t x;
    asm volatile(
        "sldt %0"
        : "=rm" (x)
...
__always_inline uint16_t lar(const uint32_t value) {
    uint32_t x;
    asm volatile(
        "lar %[x], %[value]"
        : [x] "=r" (x)
